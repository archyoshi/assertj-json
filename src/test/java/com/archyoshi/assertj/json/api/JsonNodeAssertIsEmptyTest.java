...
class JsonNodeAssertIsEmptyTest {
    ...
    void shouldFailIfJsonObjectIsNotEmpty() {
        ...
        thenThrownBy(() -> assertThat(actual).isEmpty()).isInstanceOf(AssertionError.class).hasMessageContaining("expected: 0").hasMessageContaining("but was: 1");
    }

    @Test
    void shouldFailIfJsonArrayIsNotEmpty() throws Exception {
        final JsonNode actual = mapper.readTree("[1, 2, 3]");
        thenThrownBy(() -> assertThat(actual).isEmpty()).isInstanceOf(AssertionError.class).hasMessageContaining("expected: 0").hasMessageContaining("but was: 3");
    }
}
